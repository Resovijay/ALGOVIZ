onload = function () {
    ...
    const temptext3 = document.getElementById('temptext3');
    ...
    function createData(){
        ...
        for(let i=2;i<=V;i++){
            ...
            edges.push({type: 0, from: i, to: neigh, color: '#865DFF',label: String(Math.floor(Math.random()*70)+31)});
        }
    ...
    function solveData() {
        ...
        let new_edges = [];
        if(plane!==0){
            new_edges.push({arrows: { to: { enabled: true}}, from: p1+1, to: p2+1, color: '#FB2576',label: String(plane)});
    ...
    function pushEdges(dist, curr, reverse) {
        ...
        while(dist[curr][0]!==0){
            let fm = dist[curr][1];
            if(reverse)
                tmp_edges.push({arrows: { to: { enabled: true}},from: curr+1, to: fm+1, color: '#865DFF', label: String(dist[curr][0] - dist[fm][0])});
            else
                tmp_edges.push({arrows: { to: { enabled: true}},from: fm+1, to: curr+1, color: '#865DFF', label: String(dist[curr][0] - dist[fm][0])});
            curr = fm;
        }
        return tmp_edges;
    }
